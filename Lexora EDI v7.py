...
def GenerateMapping():
    ...
    Mapping.append(Data)
    Data = []
    Data.append("Assembly Required")#Name
    Data.append(["Assembly Required"])#Alternate name list
    Data.append(None)#Location Column
    Data.append(False)#Can Autofill
    Data.append(None)#Default Value
    Mapping.append(Data)
    Data = []
    Data.append("Installation Type")#Name
    Data.append(["Installation Type"])#Alternate name list
    Data.append(None)#Location Column
    Data.append(False)#Can Autofill
    Data.append(None)#Default Value
    Mapping.append(Data)
...
def GenerateInstallationAssembly(Inputs,Outputs):
    for Input in Inputs:
        InputName = Input[0]
        InputSheet = Input[2]
        InputSheetOffset = Input[3]
        ...
        for Output in Outputs:
            ...
            OutputUPC = GetMappingData(OutputMapping,'UPC')
            InputUPC = GetMappingData(InputMapping,'UPC')
            AssemblyRequired = GetMappingData(OutputMapping,'Assembly Required')
            InstallationType = GetMappingData(OutputMapping,'Installation Type')
            AssemblyRequiredBP = GetMappingData(InputMapping,'Bullet Point 13')
            InstallationTypeBP = GetMappingData(InputMapping,'Bullet Point 14')
            OutputInstallationAssemblys = [AssemblyRequired,InstallationType]
            InputInstallationAssemblys = [AssemblyRequiredBP,InstallationTypeBP]
            IOInstallationAssemblys = OutputInstallationAssemblys+InputInstallationAssemblys
            print("Generating '"+OutputName+"' Installation/Assembly using '"+InputName+"'")
            Break = False
            for IO in IOInstallationAssemblys:
                if IO == None:
                    print("Error '"+OutputName+" Missing Installation/Assembly using '"+InputName+"'")
                    Break = True
            if Break == True:
                Break = False
                break
            for OutputRow in range(1+OutputSheetOffset,OutputSheet.max_row+1):
                ...
                for InputRow in range(1+InputSheetOffset,InputSheet.max_row+1):
                    InputUPCValue = str(InputSheet[GetColLetter(InputUPC[2])+str(InputRow)].value)
                    if OutputUPCValue != None and InputUPCValue != None and str(OutputUPCValue) == str(InputUPCValue):
                        OutputInstallationAssemblysValue = []
                        InputInstallationAssemblysValue = []
                        for Index in range(0,len(OutputInstallationAssemblys)):
                            OutputInstallationAssemblysValue.append(OutputSheet[GetColLetter(OutputInstallationAssemblys[Index][2])+str(OutputRow)].value)
                        for Index in range(0,len(InputInstallationAssemblys)):
                            if InputSheet[GetColLetter(InputInstallationAssemblys[Index][2])+str(InputRow)].value != None:
                                InputInstallationAssemblysValue.append(InputSheet[GetColLetter(InputInstallationAssemblys[Index][2])+str(InputRow)].value)
                            else:
                                print("Error '"+OutputName+" Missing "+InputInstallationAssemblys[Index][0]+" using '"+InputName+"'")
                                Break = True
                        if Break == True:
                            Break = False
                            break
                        if InputInstallationAssemblysValue[0].replace('No assembly required','N/A') !='N/A':
                            if OutputInstallationAssemblysValue[0] != None:
                                OutputSheet[GetColLetter(OutputInstallationAssemblys[0][2])+str(OutputRow)].value = 'Yes'
                                #print(OutputName+" Row: "+str(OutputRow)+" Overwrote "+OutputInstallationAssemblys[0][0]+" of UPC: "+str(OutputUPCValue)+" to "+"Old: "+InputInstallationAssemblys[0]+" New: "+'Yes')
                            else:
                                OutputSheet[GetColLetter(OutputInstallationAssemblys[0][2])+str(OutputRow)].value = 'Yes'
                                #print(OutputName+" Row: "+str(OutputRow)+" Wrote "+OutputInstallationAssemblys[0][0]+" of UPC: "+str(OutputUPCValue)+" to "+'Yes')
                        else:
                            if OutputInstallationAssemblysValue[0] != None:
                                OutputSheet[GetColLetter(OutputInstallationAssemblys[0][2])+str(OutputRow)].value = 'No'
                                #print(OutputName+" Row: "+str(OutputRow)+" Overwrote "+OutputInstallationAssemblys[0][0]+" of UPC: "+str(OutputUPCValue)+" to "+"Old: "+InputInstallationAssemblys[0]+" New: "+'No')
                            else:
                                OutputSheet[GetColLetter(OutputInstallationAssemblys[0][2])+str(OutputRow)].value = 'No'
                                #print(OutputName+" Row: "+str(OutputRow)+" Wrote "+OutputInstallationAssemblys[0][0]+" of UPC: "+str(OutputUPCValue)+" to "+'No')
                        if 'free' not in InputInstallationAssemblysValue[1].lower().replace('N/A','free'):
                            if OutputInstallationAssemblysValue[1] != None:
                                OutputSheet[GetColLetter(OutputInstallationAssemblys[1][2])+str(OutputRow)].value = 'Wall mount'
                                #print(OutputName+" Row: "+str(OutputRow)+" Overwrote "+OutputInstallationAssemblys[1][0]+" of UPC: "+str(OutputUPCValue)+" to "+"Old: "+InputInstallationAssemblys[1]+" New: "+'Wall mount')
                            else:
                                OutputSheet[GetColLetter(OutputInstallationAssemblys[1][2])+str(OutputRow)].value = 'Wall mount'
                                #print(OutputName+" Row: "+str(OutputRow)+" Wrote "+OutputInstallationAssemblys[1][0]+" of UPC: "+str(OutputUPCValue)+" to "+'Wall mount')
                        else:
                            if OutputInstallationAssemblysValue[1] != None:
                                OutputSheet[GetColLetter(OutputInstallationAssemblys[1][2])+str(OutputRow)].value = 'Freestanding'
                                #print(OutputName+" Row: "+str(OutputRow)+" Overwrote "+OutputInstallationAssemblys[1][0]+" of UPC: "+str(OutputUPCValue)+" to "+"Old: "+InputInstallationAssemblys[1]+" New: "+'Freestanding')
                            else:
                                OutputSheet[GetColLetter(OutputInstallationAssemblys[1][2])+str(OutputRow)].value = 'Freestanding'
                                #print(OutputName+" Row: "+str(OutputRow)+" Wrote "+OutputInstallationAssemblys[1][0]+" of UPC: "+str(OutputUPCValue)+" to "+'Freestanding')
                    else:
                        if OutputUPCValue == None:
                            print("Error '"+OutputName+"' Row: "+str(OutputRow)+" No UPC")
                            break

def GetNumberofDoors(BP):
    BP = BP.lower()
    Number = None
    if 'n/a' in BP:
        Number = 0
        ...
        Number = 1
    elif 'double' in BP or 'two' in BP:
        Number = 2
    elif 'four' in BP:
        Number = 4
    else:
        print('Number of Doors Error Read'+str(BP))
    return str(Number)

def GetNumberofDrawers(BP):
    BP = BP.lower()
    Number = None
    Multiplier = 1
    if 'double' in BP:
    ...
    if 'n/a' in BP:
        ...
        print('Number of Drawers Error Read'+str(BP))
    return str(Multiplier*Number)

def GenerateDoorDraws(Inputs,Outputs):
    for Input in Inputs:
        InputName = Input[0]
        InputSheet = Input[2]
        InputSheetOffset = Input[3]
        ...
        for Output in Outputs:
            ...
            OutputUPC = GetMappingData(OutputMapping,'UPC')
            InputUPC = GetMappingData(InputMapping,'UPC')
            NumberofDoors = GetMappingData(OutputMapping,'Number of Doors')
            NumberofDrawers = GetMappingData(OutputMapping,'Number of Drawers')
            NumberofDoorsBP = GetMappingData(InputMapping,'Bullet Point 10')
            NumberofDrawersBP = GetMappingData(InputMapping,'Bullet Point 11')
            OutputDoorDraws = [NumberofDoors,NumberofDrawers]
            InputDoorDraws = [NumberofDoorsBP,NumberofDrawersBP]
            IODoorDraws = OutputDoorDraws+InputDoorDraws
            print("Generating '"+OutputName+"' Door/Draws using '"+InputName+"'")
            Break = False
            for IO in IODoorDraws:
                if IO == None:
                    print("Error '"+OutputName+" Missing Door/Draws using '"+InputName+"'")
                    Break = True
            if Break == True:
                Break = False
                break
            for OutputRow in range(1+OutputSheetOffset,OutputSheet.max_row+1):
                ...
                for InputRow in range(1+InputSheetOffset,InputSheet.max_row+1):
                    InputUPCValue = str(InputSheet[GetColLetter(InputUPC[2])+str(InputRow)].value)
                    if OutputUPCValue != None and InputUPCValue != None and str(OutputUPCValue) == str(InputUPCValue):
                        OutputDoorDrawsValue = []
                        InputDoorDrawsValue = []
                        for Index in range(0,len(OutputDoorDraws)):
                            OutputDoorDrawsValue.append(OutputSheet[GetColLetter(OutputDoorDraws[Index][2])+str(OutputRow)].value)
                        for Index in range(0,len(InputDoorDraws)):
                            if InputSheet[GetColLetter(InputDoorDraws[Index][2])+str(InputRow)].value != None:
                                InputDoorDrawsValue.append(InputSheet[GetColLetter(InputDoorDraws[Index][2])+str(InputRow)].value)
                            else:
                                print("Error '"+OutputName+" Missing "+InputDoorDraws[Index][0]+" using '"+InputName+"'")
                                Break = True
                        if Break == True:
                            Break = False
                            break
                        NumberofDoorsString = GetNumberofDoors(InputDoorDrawsValue[0])
                        NumberofDrawersString = GetNumberofDrawers(InputDoorDrawsValue[1])
                        if OutputDoorDrawsValue[0] != None:
                            OutputSheet[GetColLetter(OutputDoorDraws[0][2])+str(OutputRow)].value = NumberofDoorsString
                            #print(OutputName+" Row: "+str(OutputRow)+" Overwrote "+OutputDoorDraws[0][0]+" of UPC: "+str(OutputUPCValue)+" to "+"Old: "+OutputDoorDrawsValue[0]+" New: "+NumberofDoorsString)
                        else:
                            OutputSheet[GetColLetter(OutputDoorDraws[0][2])+str(OutputRow)].value = NumberofDoorsString
                            #print(OutputName+" Row: "+str(OutputRow)+" Wrote "+OutputDoorDraws[0][0]+" of UPC: "+str(OutputUPCValue)+" to "+NumberofDoorsString)
                        if OutputDoorDrawsValue[0] != None:
                            OutputSheet[GetColLetter(OutputDoorDraws[1][2])+str(OutputRow)].value = NumberofDrawersString
                            #print(OutputName+" Row: "+str(OutputRow)+" Overwrote "+OutputDoorDraws[1][0]+" of UPC: "+str(OutputUPCValue)+" to "+"Old: "+OutputDoorDrawsValue[1]+" New: "+NumberofDrawersString)
                        else:
                            OutputSheet[GetColLetter(OutputDoorDraws[1][2])+str(OutputRow)].value = NumberofDrawersString
                            #print(OutputName+" Row: "+str(OutputRow)+" Wrote "+OutputDoorDraws[1][0]+" of UPC: "+str(OutputUPCValue)+" to "+NumberofDrawersString)
                    else:
                        if OutputUPCValue == None:
                            print("Error '"+OutputName+"' Row: "+str(OutputRow)+" No UPC")
                            break
...
def main():
    ...
    GenerateInstallationAssembly(Inputs,Outputs)
    GenerateDoorDraws(Inputs,Outputs)
    #Generators
